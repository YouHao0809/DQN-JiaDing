...
class Agent(object):
    ...
    def fit(self, env, nb_steps, action_repetition=1, callbacks=None, verbose=1,
        ...
        logpath = './rewards_{}.log'.format(stime)
        ...
        try:
            while self.step < nb_steps:
                print('*' * 50)
                print('nb_step:{}'.format(self.step))
                print('*' * 50)
                if observation is None:  # start of a new episode
                    ...
                    for _ in range(nb_random_start_steps):
                        ...
                        observation, reward, done, info = env.step(action, logpath)
                        # 修改
                        action = env.orp_action if env.orp_action is not None else action
                ...
                for _ in range(action_repetition):
                    ...
                    observation, r, done, info = env.step(action, logpath)
                    observation = deepcopy(observation)
                    if self.processor is not None:
                ...
                # 保存权重
                if self.step > 0 and self.step % INT_SAVE == 0:
                    self.save_weights('weights/weights_after_step_{}.h5f'.format(self.step), overwrite=True)
                episode_step += 1
                self.step += 1
