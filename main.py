...
if __name__ == "__main__":
	...
	env = RebalancingEnv( Model(DMD_MAT, DMD_VOL, V=FLEET_SIZE, K=VEH_CAPACITY, rebl='no', reopt='no'),
						  penalty=-0, osrm=osrm)
	...
	dqn.save_weights('weights/final_weights.b5f', overwrite=True)

	# dqn.load_weights('weights/weights_at_step_100.h5f')
	...
	# 		model = Model(DMD_MAT, DMD_VOL, dqn=dqn, V=FLEET_SIZE, K=VEH_CAPACITY, assign=MET_ASSIGN,
	# 					  reopt=MET_REOPT, rebl=MET_REBL)
	# 		# start time
	# 		stime = time.time()
