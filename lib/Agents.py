...
class Veh(object):
    ...
    def __init__(self, id, rs, K=4, S=6, T=0.0, lng=None, lat=None):
        self.id = id
        self.idle = True
        self.rebl = False
        self.T = T
        self.lng = (Olng + Dlng) / 2 + (Dlng - Olng) * rs.uniform(-0.35, 0.35) if lng is None else lng
        self.lat = (Olat + Dlat) / 2 + (Dlat - Olat) * rs.uniform(-0.35, 0.35) if lat is None else lat
        self.org_lng = self.lng
    ...
    def get_position(self):
        i = (Dlat - self.lat) // Elat
        j = (self.lng - Olng) // Elng
        return int(i), int(j)
            ...
            if leg.t < dT:
                ...
                while dT > 0 and len(leg.steps) > 0:
                    ...
                    if step.t < dT:
                        ...
                        # self.jump_to_location(leg.tlng, leg.tlat)
                        self.jump_to_location(step.geo[-1][0], step.geo[-1][1])  # 修改
                        self.pop_step()
                        if len(leg.steps) == 0:
                    ...
                    # the vehicle has to stop somewhere within the step
                        ...
                        # self.cut_step(pct)
                        lng, lat = self.cut_step(pct)  # 修改
                        # self.jump_to_location(step.geo[0][0], step.geo[0][1])
                        self.jump_to_location(lng, lat)  # 修改
                        self.T = T
                        return done
    ...
    def cut_step(self, pct):
        ...
        return step.geo[0][0], step.geo[0][1]  # 修改
...
class Model(object):
    ...
    def __init__(self, M, D, dqn=None, V=2, K=4, assign="ins", reopt="no", rebl="no"):
        ...
        # 修改：把车放在订单多的位置
        for i in range(V):
            self.vehs.append(Veh(i, self.rs2, K=K, lng=M_MIT[i][0], lat=M_MIT[i][1]))
        self.N = 0
        self.reqs = []
        self.rejs = []
        self.queue = deque([])
    ...
    def dispatch_at_time(self, osrm, T):
        ...
        for veh in self.vehs:
            ...
            # if self.T > 300 and np.isclose(veh.lat, veh.org_lat) and np.isclose(veh.lng, veh.org_lng):
            #     print('不动的车的id,', veh.id)
            # if veh.id == 9:
    ...
    # 使用orp计算出action
    def get_orp_action(self, osrm, T):
        d = np.zeros((Nlat, Nlng))
        c = np.zeros((Nlat, Nlng, 2))
        v = np.zeros((Nlat, Nlng))
        s = np.zeros((Nlat, Nlng))
        ...
        # 区域索引变换,b和c换为以车为中心
        for i, j in itertools.product(range(Nlat), range(Nlng)):
            if (lat - Mlat * Elat / 2) < (Elat * i + Olat) < (lat + Mlat * Elat / 2) and \
                    (lng - Mlng * Elng / 2) < (Elng * j - Olng) < (lng + Mlng * Elng / 2):
                b[i][j] = 0
                c[i][j] = 0
        i, j = np.unravel_index(b.argmax(), b.shape)  # 再平衡到概率最大的车辆感知区域
        route = [(-1, 0, c[i][j][0], c[i][j][1])]
        r = int((Dlat - lat) // Elat)
        c = int((lng - Olng) // Elng)
        action = None
    ...
    def get_state(self, veh):
        ...
        p = np.zeros((Mlat, Mlng))  # 修改：车辆位置（用1表示，其他为0）
        ...
        for m in self.M:
            for i, j in itertools.product(range(Mlat), range(Mlng)):
                ...
                if Olng + j * Elng <= m[0] <= Olng + (1 + j) * Elng:
                    if Dlat - i * Elat >= m[1] >= Dlat - (i + 1) * Elat:
                        d[i][j] += m[4] * self.D
                        c[i][j][0] += m[0] * m[4] * self.D
                        c[i][j][1] += m[1] * m[4] * self.D
                        break
        ...
        for veh_ in self.vehs:
            if veh_.idle:
                for i, j in itertools.product(range(Mlat), range(Mlng)):
                    # if veh_.lat <= lat + Mlat * Elat / 2 - i * Elat and veh_.lat >= lat + Mlat * Elat / 2 - (
                    #         i + 1) * Elat:
                    #     if veh_.lng >= lng - Mlng * Elng / 2 + j * Elng and veh_.lng <= lng - Mlng * Elng / 2 + (
                    #             j + 1) * Elng:
                    if Olng + j * Elng <= veh_.lng <= Olng + (1 + j) * Elng:
                        if Dlat - i * Elat >= veh_.lat >= Dlat - (i + 1) * Elat:
                            v[i][j] += 1
                            break
            else:
                lng_, lat_, n = veh_.get_location_at_time(self.T + INT_REBL)
                for i, j in itertools.product(range(Mlat), range(Mlat)):
                    if Olng + j * Elng <= lng_ <= Olng + (1 + j) * Elng:
                        if Dlat - i * Elat >= lat_ >= Dlat - (i + 1) * Elat:
                            if n == 0:
                                s[i][j] += 0.8
                            elif n == 1:
                                s[i][j] += 0.4
